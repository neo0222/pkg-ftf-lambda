...
async function truncateMaterial() {
  const items = await findAll(materialTableName);
  await deleteAllById(materialTableName, items);
}

async function truncateIngredient() {
  const items = await findAll(ingredientTableName);
  await deleteAllById(ingredientTableName, items);
}

async function truncateBaseItem() {
  const items = await findAll(baseItemTableName);
  await deleteAllById(baseItemTableName, items);
}

async function truncateProduct() {
  const items = await findAll(productTableName);
  await deleteAllById(productTableName, items);
}

async function truncateStock() {
  const items = await findAll(stockTableName);
  await deleteAllByFoodTypeAndId(stockTableName, items);
}

async function truncateFlow() {
  const items = await findAll(flowTableName);
  await deleteAllByFoodTypeAndDate(flowTableName, items);
}

async function truncateSequence() {
  const items = await findAll(sequenceTableName);
  await deleteAllByTableName(sequenceTableName, items);
}
...
async function deleteAllByTableName(tableName, items) {
  const promises = [];
  for (const item of items) {
    promises.push((async () => {
      const params = {
...
async function deleteAllByFoodTypeAndDate(tableName, items) {
  const promises = [];
  for (const item of items) {
    promises.push((async () => {
      const params = {
